"""
Gemini fine-tuning provider (simulated).
"""
...
class GeminiProvider(BaseProvider):
    ...
    async def start_finetune(self, config: FineTuneConfig, train_path: Path, validation_path: Path) -> str:
        job_id = f"ft-gemini-{uuid4().hex[:8]}"
        model_name = f"gemini-{config.base_model}-synthetic-ui-{uuid4().hex[:6]}"
        self._jobs[job_id] = {
    ...
    async def download_model(self, model_name: str, output_path: Path) -> Path:
        ...
        metadata = {"provider": "gemini", "model_name": model_name}
